...
import { 
  ...
  Sparkles, PlayCircle, Loader2, Edit2, Lock, CheckCircle2
} from 'lucide-react';
...
const AdminPanel = ({ onBack }: { onBack: () => void }) => {
  ...
  const [upContext, setUpContext] = useState<{type: FeedbackType | 'ref', index?: number} | null>(null);
  ...
  if (loading) return <div className="h-full bg-white flex flex-col items-center justify-center space-y-4"><Loader2 className="w-10 h-10 animate-spin text-blue-500" /><p className="font-bold text-slate-400 text-sm">正在加载...</p></div>;
  ...
  if (viewMode === 'form') return (
    <div className="h-full bg-white flex flex-col p-6 space-y-6 overflow-y-auto pb-24 animate-in slide-in-from-bottom duration-300">
      <div className="flex items-center gap-4">
        ...
        <h2 className="text-xl font-bold">配置规则</h2>
      </div>
      <div className="space-y-6">
        <div className="space-y-2"><label className="text-xs font-bold text-slate-400">1. 名称</label><input className="w-full p-5 bg-slate-50 rounded-3xl border-none font-bold outline-none" value={form.name} onChange={e => setForm({...form, name: e.target.value})} placeholder="例如：我的架子鼓" /></div>
        
        <div className="space-y-3">
          <label className="text-xs font-bold text-slate-400">2. 识别详细定义 (文字 + 参考图)</label>
          <div className="bg-slate-50 p-5 rounded-[2.5rem] space-y-4 border-2 border-dashed border-slate-100">
            <textarea className="w-full p-4 bg-white rounded-2xl text-sm border-none shadow-sm outline-none h-24" value={form.targetValue} onChange={e => setForm({...form, targetValue: e.target.value})} placeholder="输入文字特征描述..." />
            <button onClick={() => { setUpContext({type:'ref'}); mediaRef.current?.click(); }} className={`w-full p-4 rounded-2xl flex items-center justify-center gap-2 text-sm font-bold shadow-sm transition-all ${form.referenceImage ? 'bg-blue-600 text-white' : 'bg-white text-slate-600'}`}>
              <Camera className="w-4 h-4" /> {form.referenceImage ? '比对参考图已就绪' : '上传比对参考图'}
            </button>
          </div>
        </div>
          ...
          <div className="bg-slate-50 p-5 rounded-[2.5rem] space-y-4">
            ...
            <div className="grid grid-cols-1 gap-2">
              {['image', 'video', 'audio'].map((t: any) => {
                ...
                return (
                  <button key={t} onClick={() => { setUpContext({type:t}); mediaRef.current?.click(); }} className={`w-full p-4 rounded-2xl flex items-center justify-center gap-2 text-sm font-bold shadow-sm transition-all ${active ? 'bg-blue-600 text-white' : 'bg-white text-slate-600'}`}>
                    {t === 'image' && <ImageIcon className="w-4 h-4" />}
                    {t === 'video' && <Video className="w-4 h-4" />}
                    {t === 'audio' && <Music className="w-4 h-4" />}
                    {active ? `${t === 'image' ? '图片' : t === 'video' ? '视频' : '音频'}已就绪` : `上传反馈${t === 'image' ? '图片' : t === 'video' ? '视频' : '音频'}`}
                  </button>
                );
              })}
            </div>
        ...
        {uploading && <div className="text-center text-blue-500 text-xs animate-pulse font-bold">处理中...</div>}
        <button onClick={async () => { await storageService.saveRule(form); loadData(); setViewMode('list'); }} className="w-full bg-blue-600 text-white p-6 rounded-[2.5rem] font-bold shadow-xl active:scale-95 text-lg">保存规则</button>
      </div>
    </div>
  );
    ...
    <div className="h-full flex flex-col p-6 bg-white overflow-hidden animate-in fade-in">
      ...
      <div className="flex-1 overflow-y-auto space-y-4 scrollbar-hide pb-20">
        <button onClick={() => { setForm({ name: '', targetValue: '', feedback: [{type:'text', content:''}] }); setViewMode('form'); }} className="w-full p-10 border-4 border-dashed border-slate-100 rounded-[3.5rem] flex flex-col items-center justify-center gap-2 text-slate-300 active:text-blue-400">
          <Plus className="w-8 h-8" /><span className="font-bold text-xs">新增规则</span>
        </button>
        {localRules.map((rule: any) => (
          <div key={rule.objectId || rule.id} className="p-5 bg-white border border-slate-100 rounded-[2.5rem] flex items-center gap-4 shadow-sm active:bg-slate-50 transition-all">
            ...
            <div className="flex-1 overflow-hidden">
              ...
              <div className="text-[10px] text-slate-400 truncate">{rule.targetValue}</div>
            </div>
            <div className="flex gap-1">
              <button onClick={() => { setForm(rule); setViewMode('form'); }} className="p-2 text-blue-300"><Edit2 className="w-4 h-4" /></button>
              <button onClick={() => { if(confirm('确定删除？')) storageService.deleteRule(rule.objectId).then(loadData); }} className="p-2 text-red-200"><Trash2 className="w-4 h-4" /></button>
            </div>
          </div>
        ))}
      </div>
    ...
    <div className="fixed inset-0 z-[999] flex items-center justify-center p-6 animate-in fade-in duration-300">
      {capturedImage && (
        <div className="absolute inset-0 z-0 overflow-hidden">
           <img src={capturedImage} className="w-full h-full object-cover scale-110 blur-xl opacity-60" alt="bg" />
      ...
      <div className="z-10 w-full max-w-md space-y-6 flex flex-col items-center max-h-[90vh] overflow-y-auto scrollbar-hide pb-20">
        {result ? (
          <>
            ...
            {result.feedback.map((fb: any, i: number) => (
              <div key={i} className="w-full bg-white/95 backdrop-blur-xl rounded-[3rem] p-8 shadow-2xl animate-in slide-in-from-bottom duration-700">
                ...
                {fb.type === 'video' && <video src={fb.content} autoPlay playsInline controls className="w-full rounded-[2rem] bg-black" />}
            ...
            <button onClick={onClose} className="w-full bg-white/20 text-white p-6 rounded-[2.5rem] font-bold text-2xl backdrop-blur-3xl border border-white/20 active:scale-95 mt-4">确认返回</button>
          </>
        ) : (
          <div className="bg-white/95 backdrop-blur-xl p-12 rounded-[4rem] text-center space-y-8 shadow-2xl animate-in zoom-in">
             ...
             <div className="space-y-2"><p className="text-3xl font-bold text-slate-800">未匹配</p><p className="text-slate-400 text-xs break-all opacity-80 px-4">{error}</p></div>
...
export default function App() {
  ...
  if (view === 'admin-login') return (
    <div className="h-dvh bg-white flex flex-col items-center justify-center p-8 space-y-8 animate-in fade-in">
      ...
      <input type="password" pattern="[0-9]*" inputMode="numeric" autoFocus className="w-full max-w-xs p-6 bg-slate-50 rounded-[2.5rem] text-center text-3xl font-bold tracking-[0.5em] outline-none" onChange={e => e.target.value === '11335510' && setView('admin')} />
      <button onClick={() => setView('home')} className="text-slate-300 font-bold">取消</button>
    </div>
  );
      ...
      <main className="flex-1 relative overflow-hidden">
        {view === 'home' && (
          <div className="flex flex-col items-center justify-center h-full px-10 space-y-12 animate-in fade-in duration-700">
            <div className="w-36 h-36 bg-blue-600 rounded-[3.5rem] flex items-center justify-center shadow-2xl shadow-blue-200 animate-bounce-slow"><Camera className="w-16 h-16 text-white" /></div>
            ...
            <div className="w-full space-y-5 max-w-xs pb-10">
              <button onClick={() => setView('camera')} className="w-full bg-blue-600 text-white p-6 rounded-[2.5rem] shadow-xl active:scale-95 font-bold text-xl">开启相机</button>
        ...
        {view === 'processing' && (
          <div className="flex flex-col items-center justify-center h-full space-y-10 bg-white absolute inset-0 z-[120] animate-in fade-in">
             ...
             <p className="text-2xl font-bold text-slate-800 tracking-tight">正在极速分析...</p>
          </div>
        )}
