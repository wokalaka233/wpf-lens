...
// --- 1. 反馈视图 (全屏覆盖 / 照片背景 / blur(15px) / 深色遮罩) ---
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6 animate-in fade-in duration-300">
      ...
      {capturedImage && (
        <div className="absolute inset-0 z-0 overflow-hidden bg-black">
           {/* 背景照片：filter blur(15px) */}
           <img 
             src={capturedImage} 
             className="w-full h-full object-cover scale-110 filter blur-[15px] opacity-70" 
             alt="反馈背景" 
           />
           {/* 深色遮罩：rgba(0,0,0,0.4) 防止底层 UI 穿透 */}
        ...
        {result ? (
          ...
          <div className="bg-white/95 backdrop-blur-xl p-12 rounded-[4rem] text-center space-y-8 shadow-2xl animate-in zoom-in">
             ...
             <div className="space-y-2">
                ...
                <p className="text-slate-400 font-medium text-xs break-all opacity-80 px-4">分析详情：{error}</p>
             </div>
...
const AdminPanel = ({ onBack }: { onBack: () => void }) => {
  ...
  if (viewMode === 'form') return (
    <div className="h-full bg-white flex flex-col p-6 space-y-6 overflow-y-auto pb-24 animate-in slide-in-from-bottom duration-300">
      <div className="flex items-center gap-4"><button onClick={() => setViewMode('list')} className="p-2 bg-slate-50 rounded-xl"><ChevronLeft /></button><h2 className="text-xl font-bold">配置规则</h2></div>
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="bg-slate-50 p-5 rounded-[2.5rem] space-y-4 border-2 border-dashed border-slate-200">
            <textarea className="w-full p-4 bg-white rounded-2xl text-sm border-none shadow-sm outline-none h-24" value={form.targetValue} onChange={e => setForm({...form, targetValue: e.target.value})} placeholder="输入详细特征描述词..." />
            <button onClick={() => { setUpContext({type:'ref'}); mediaRef.current?.click(); }} className={`w-full p-4 rounded-2xl flex items-center justify-center gap-2 text-sm font-bold transition-all ${form.referenceImage ? 'bg-blue-600 text-white shadow-xl shadow-blue-200' : 'bg-white text-slate-600'}`}>
              <Camera className="w-4 h-4" /> {form.referenceImage ? '参考图就绪' : '上传比对图'}
            </button>
          </div>
        </div>
        <div className="space-y-3">
          <label className="text-xs font-bold text-slate-400">3. 反馈媒体 (蓝色为已上传)</label>
          <div className="bg-slate-50 p-5 rounded-[2.5rem] space-y-4">
            ...
            <div className="grid grid-cols-1 gap-2">
              {['image', 'video', 'audio'].map((t: any) => {
                const active = form.feedback?.some((f: any) => f.type === t && f.content?.startsWith('http'));
                return (
                  <button key={t} onClick={() => { setUpContext({type:t}); mediaRef.current?.click(); }} className={`w-full p-4 rounded-2xl flex items-center justify-center gap-2 text-sm font-bold shadow-sm transition-all ${active ? 'bg-blue-600 text-white shadow-xl shadow-blue-200' : 'bg-white text-slate-600'}`}>
                    {t === 'image' ? <ImageIcon className="w-4 h-4" /> : t === 'video' ? <Video className="w-4 h-4" /> : <Music className="w-4 h-4" />}
                    {active ? `${t === 'image' ? '图片' : t === 'video' ? '视频' : '音频'}已就绪` : `上传反馈${t}`}
                  </button>
                );
              })}
            </div>
        ...
        {uploading && <div className="text-center text-blue-500 text-xs animate-pulse font-bold tracking-widest">正在上传到云端...</div>}
...
export default function App() {
  ...
  if (view === 'admin-login') return (
    <div className="h-dvh bg-white flex flex-col items-center justify-center p-8 space-y-8 animate-in fade-in">
      ...
      <h2 className="text-3xl font-bold">安全登录</h2>
        ...
        {view === 'processing' && (
          <div className="flex flex-col items-center justify-center h-full space-y-10 bg-white absolute inset-0 z-[120] animate-in fade-in">
             <div className="relative w-40 h-40 flex items-center justify-center">
                ...
                {capturedImage && <img src={capturedImage} className="w-32 h-32 object-cover rounded-full shadow-lg border-4 border-white animate-pulse" />}
             </div>
             <p className="text-2xl font-bold text-slate-800 tracking-tight text-center px-6">正在极速比对画面...</p>
          </div>
        )}
