...
const AdminPanel = ({ onBack }: { onBack: () => void }) => {
  ...
  const hasMedia = (type: string) => form.feedback?.some((f: any) => f.type === type && f.content?.startsWith('http'));
  ...
  if (loading) return <div className="h-full flex flex-col items-center justify-center space-y-4 bg-white"><Loader2 className="w-10 h-10 animate-spin text-blue-500" /><p className="font-bold text-slate-400 text-sm">规则库同步中...</p></div>;
  ...
  if (viewMode === 'form') return (
    <div className="h-full bg-white flex flex-col p-6 space-y-6 overflow-y-auto pb-24 animate-in slide-in-from-bottom duration-300">
      <div className="flex items-center gap-4"><button onClick={() => setViewMode('list')} className="p-2 bg-slate-50 rounded-xl"><ChevronLeft /></button><h2 className="text-xl font-bold">配置识别规则</h2></div>
      <div className="space-y-6">
        <div className="space-y-2"><label className="text-xs font-bold text-slate-400 uppercase tracking-wider">1. 规则名称</label><input className="w-full p-5 bg-slate-50 rounded-3xl border-none font-bold outline-none" value={form.name} onChange={e => setForm({...form, name: e.target.value})} placeholder="例如：我的架子鼓" /></div>
        <div className="space-y-3">
          <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">2. 识别详细定义 (文字 + 参考图)</label>
          <div className="bg-slate-50 p-5 rounded-[2.5rem] space-y-4 border-2 border-dashed border-slate-200">
            <textarea className="w-full p-4 bg-white rounded-2xl text-sm border-none shadow-sm outline-none h-24" value={form.targetValue} onChange={e => setForm({...form, targetValue: e.target.value})} placeholder="输入文字描述特征..." />
            <button onClick={() => { setUpContext({type:'ref'}); mediaRef.current?.click(); }} className={`w-full p-4 rounded-2xl flex items-center justify-center gap-2 text-sm font-bold shadow-sm transition-all ${form.referenceImage ? 'bg-blue-600 text-white shadow-blue-200' : 'bg-white text-slate-600'}`}>
              <Camera className="w-4 h-4" /> {form.referenceImage ? '比对图已就绪' : '上传比对参考图'}
            </button>
          </div>
        </div>
        <div className="space-y-3">
          <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">3. 触发反馈 (蓝色表示已就绪)</label>
          <div className="bg-slate-50 p-5 rounded-[2.5rem] space-y-4">
            ...
            <div className="grid grid-cols-1 gap-2">
              {['image', 'video', 'audio'].map((t: any) => (
                <button key={t} onClick={() => { setUpContext({type:t}); mediaRef.current?.click(); }} className={`w-full p-4 rounded-2xl flex items-center justify-center gap-2 text-sm font-bold shadow-sm transition-all ${hasMedia(t) ? 'bg-blue-600 text-white' : 'bg-white text-slate-600'}`}>
                  {t === 'image' ? <ImageIcon className="w-4 h-4" /> : t === 'video' ? <Video className="w-4 h-4" /> : <Music className="w-4 h-4" />}
                  {hasMedia(t) ? `${t === 'image' ? '图片' : t === 'video' ? '视频' : '音频'}已上传` : `上传反馈${t === 'image' ? '图片' : t === 'video' ? '视频' : '音频'}`}
                </button>
              ))}
            </div>
          </div>
        </div>
        {uploading && <div className="text-center text-blue-500 text-xs animate-pulse font-bold tracking-widest">同步媒体文件中...</div>}
        <button onClick={async () => { try { await storageService.saveRule(form); loadData(); setViewMode('list'); } catch(e:any) { alert(e.message); } }} className="w-full bg-blue-600 text-white p-6 rounded-[2.5rem] font-bold shadow-xl active:scale-95 text-lg">保存规则</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="h-full flex flex-col p-6 bg-white overflow-hidden animate-in fade-in">
      <div className="flex justify-between items-center mb-6"><button onClick={onBack} className="p-2 bg-slate-50 rounded-xl"><ChevronLeft /></button><h2 className="text-xl font-bold">规则管理</h2><div className="w-10"></div></div>
      <div className="flex-1 overflow-y-auto space-y-4 scrollbar-hide pb-20">
        <button onClick={() => { setForm({ name: '', targetValue: '', feedback: [{type:'text', content:''}] }); setViewMode('form'); }} className="w-full p-10 border-4 border-dashed border-slate-100 rounded-[3.5rem] flex flex-col items-center justify-center gap-2 text-slate-300 active:text-blue-400 transition-all"><Plus className="w-8 h-8" /><span className="font-bold text-xs">新增规则</span></button>
...
const FeedbackView = ({ result, error, capturedImage, processingTime, onClose }: any) => {
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6 animate-in fade-in duration-300 overflow-hidden">
      {capturedImage && (
        ...
        {result ? (
          <>
            <div className="text-center text-white drop-shadow-2xl py-6 animate-in zoom-in duration-500">
              ...
              <p className="text-[10px] opacity-60 font-bold tracking-[0.3em] uppercase">识别成功 · {processingTime.toFixed(2)}秒</p>
            </div>
            {result.feedback.map((fb: any, i: number) => (
              <div key={i} className="w-full bg-white/95 backdrop-blur-xl rounded-[3rem] p-8 shadow-2xl animate-in slide-in-from-bottom duration-700">
                ...
                {fb.type === 'audio' && <div className="flex items-center gap-4 bg-slate-50 p-4 rounded-[2.5rem] shadow-inner"><audio src={fb.content} autoPlay controls className="flex-1 h-10" /></div>}
              </div>
            ))}
          ...
          <div className="bg-white/95 backdrop-blur-xl p-12 rounded-[4rem] text-center space-y-8 shadow-2xl animate-in zoom-in">
             ...
             <div className="space-y-2"><p className="text-3xl font-bold text-slate-800">未匹配到规则</p><p className="text-slate-400 font-medium text-xs break-all opacity-80 px-4">系统分析：{error}</p></div>
             <button onClick={onClose} className="w-full bg-slate-900 text-white py-6 rounded-[2.5rem] font-bold text-xl active:scale-95 transition-all">返回重试</button>
          </div>
        )}
      </div>
    </div>
...
export default function App() {
  ...
  useEffect(() => { loadRules(); localAiService.loadModels(); }, []);
  ...
  if (view === 'admin-login') return (
    <div className="h-dvh bg-white flex flex-col items-center justify-center p-8 space-y-8 animate-in fade-in">
      ...
      <h2 className="text-3xl font-bold text-slate-800 tracking-tight">管理认证</h2>
      <input type="password" pattern="[0-9]*" inputMode="numeric" autoFocus className="w-full max-w-xs p-8 bg-slate-50 rounded-[2.5rem] text-center text-4xl font-bold tracking-[0.5em] outline-none focus:ring-4 ring-blue-50 transition-all" onChange={e => e.target.value === '11335510' && setView('admin')} />
      <button onClick={() => setView('home')} className="text-slate-300 font-bold uppercase tracking-widest text-xs">取消</button>
    </div>
  );
  if (view === 'admin') return <AdminPanel onBack={() => { loadRules(); setView('home'); }} />;
  ...
  return (
    <div className="h-dvh bg-white text-slate-900 font-sans max-w-lg mx-auto relative overflow-hidden flex flex-col">
      <header className="p-6 pt-safe flex justify-between items-center z-10 shrink-0 border-b border-slate-50">
        <h1 className="text-2xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent italic tracking-tight">wpf的镜头</h1>
      ...
      <main className="flex-1 relative overflow-hidden">
        {view === 'home' && (
          <div className="flex flex-col items-center justify-center h-full px-10 space-y-12 animate-in fade-in duration-700">
            ...
            <div className="w-full space-y-5 max-w-xs pb-10">
              ...
              <button onClick={() => setView('upload')} className="w-full bg-slate-50 text-slate-400 p-6 rounded-[2.5rem] active:scale-95 text-xl font-bold">上传图片</button>
            </div>
          </div>
        )}
        ...
        {view === 'processing' && (
          <div className="flex flex-col items-center justify-center h-full space-y-10 bg-white absolute inset-0 z-[120] animate-in fade-in">
             ...
             <p className="text-2xl font-bold text-slate-800 tracking-tight">正在识别中...</p>
          </div>
        )}
  ...
  return (
    <div className="relative h-full flex flex-col">
      ...
      <div className="p-10 pb-safe-offset-4 flex justify-between items-center bg-black/40 backdrop-blur-2xl absolute bottom-0 left-0 right-0">
        ...
        <button onClick={() => setIsFront(!isFront)} className="p-5 bg-white/10 rounded-full text-white active:scale-90 transition-all"><RefreshCw /></button>
      </div>
    </div>
  );
};
...
const UploadView = ({ onUpload, onClose }: any) => {
  ...
  return <div className="fixed inset-0 bg-white z-[160] flex flex-col items-center justify-center p-10"><Loader2 className="w-12 h-12 text-blue-600 animate-spin" /><p className="font-bold text-slate-400 mt-4 tracking-widest text-xs uppercase">Opening Gallery</p></div>;
};
