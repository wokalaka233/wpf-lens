  ...
  return (
    <div className="h-dvh bg-white flex flex-col items-center justify-center p-8 space-y-8 animate-in fade-in">
      ...
      <div className="text-center">
        <h2 className="text-3xl font-black italic">管理登录</h2>
        <p className="text-slate-400 text-xs font-bold mt-1 tracking-widest uppercase opacity-50">Admin Authentication</p>
      </div>
      <input 
        type="password" pattern="[0-9]*" inputMode="numeric" autoFocus
        className="w-full max-w-xs p-8 bg-slate-50 rounded-[2.5rem] text-center text-4xl font-black tracking-[0.5em] focus:ring-4 ring-blue-50 outline-none transition-all"
        onChange={e => e.target.value === '11335510' && onSuccess()} 
      />
      <button onClick={onBack} className="text-slate-300 font-bold">CANCEL</button>
    </div>
  );
};

// --- 工具组件：管理后台主面板 (恢复完整 UI) ---
const AdminPanel = ({ rules, onBack }: { rules: RecognitionRule[], onBack: () => void }) => {
  ...
  const startAdd = () => { 
    setEditingId(null); 
    setForm({ name: '', targetValue: '', feedback: [{type:'text', content:''}] }); 
    setViewMode('form'); 
  };
  ...
  if (viewMode === 'form') return (
    <div className="h-full bg-white flex flex-col p-6 space-y-6 overflow-y-auto pb-24">
      <div className="flex items-center gap-4">
        ...
        <h2 className="text-xl font-black">配置规则</h2>
      </div>
      <div className="space-y-6">
        <div className="space-y-2"><label className="text-xs font-bold text-slate-400 uppercase">1. 规则名称</label><input className="w-full p-5 bg-slate-50 rounded-3xl border-none font-bold outline-none" value={form.name} onChange={e => setForm({...form, name: e.target.value})} placeholder="例如：我的鼠标" /></div>
        <div className="space-y-2"><label className="text-xs font-bold text-slate-400 uppercase">2. 识别详细描述</label><textarea className="w-full p-5 bg-slate-50 rounded-3xl border-none h-28 outline-none" value={form.targetValue} onChange={e => setForm({...form, targetValue: e.target.value})} placeholder="描述细节，供AI比对..." /></div>
        <div className="space-y-3">
          <label className="text-xs font-bold text-slate-400 uppercase">3. 反馈内容 (可多选)</label>
          <div className="bg-slate-50 p-5 rounded-[2.5rem] space-y-4">
            ...
            <div className="grid grid-cols-1 gap-2">
              <button onClick={() => { setUpType('image'); mediaRef.current?.click(); }} className="w-full p-4 bg-white rounded-2xl flex items-center justify-center gap-2 text-sm font-bold shadow-sm active:scale-95 transition-all"><ImageIcon className="w-4 h-4" /> 上传反馈图片</button>
              <button onClick={() => { setUpType('video'); mediaRef.current?.click(); }} className="w-full p-4 bg-white rounded-2xl flex items-center justify-center gap-2 text-sm font-bold shadow-sm active:scale-95 transition-all"><Video className="w-4 h-4" /> 上传反馈视频</button>
              <button onClick={() => { setUpType('audio'); mediaRef.current?.click(); }} className="w-full p-4 bg-white rounded-2xl flex items-center justify-center gap-2 text-sm font-bold shadow-sm active:scale-95 transition-all"><Music className="w-4 h-4" /> 上传反馈音频</button>
            </div>
          </div>
        </div>
        {uploading && <div className="text-center text-blue-500 text-xs animate-pulse font-bold">正在上传到阿里云 OSS...</div>}
        <button onClick={async () => { await storageService.saveRule(form); onBack(); }} className="w-full bg-blue-600 text-white p-6 rounded-[2.5rem] font-black shadow-xl active:scale-95 transition-all">保存规则</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="h-full flex flex-col p-6 bg-white overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto space-y-4 scrollbar-hide">
        <button onClick={startAdd} className="w-full p-10 border-4 border-dashed border-slate-100 rounded-[3.5rem] flex flex-col items-center justify-center gap-2 text-slate-300 hover:text-blue-400 transition-all">
          <Plus className="w-8 h-8" /><span className="font-bold text-xs tracking-widest uppercase">新增本地测试规则</span>
        </button>
        {rules.map(rule => (
          <div key={rule.id} className="p-5 bg-white border border-slate-100 rounded-[2.5rem] flex items-center gap-4 shadow-sm active:bg-slate-50 transition-all">
            ...
            <div className="flex-1 overflow-hidden"><div className="font-bold text-slate-800 truncate">{rule.name}</div><div className="text-[10px] text-slate-400 break-all line-clamp-1">{rule.targetValue}</div></div>
            {!rule.id.startsWith('rule_') && (
              <div className="flex gap-1">
                <button onClick={() => { setEditingId(rule.id); setForm(rule); setViewMode('form'); }} className="p-2 text-blue-300"><Edit2 className="w-4 h-4" /></button>
                <button onClick={() => storageService.deleteRule(rule.id).then(onBack)} className="p-2 text-red-200"><Trash2 className="w-4 h-4" /></button>
              </div>
            )}
          </div>
        ))}
        ...
        {view === 'home' && (
          <div className="flex flex-col items-center justify-center h-full px-10 space-y-12 animate-in fade-in duration-700 pb-safe">
            <div className="relative group">
              <div className="w-36 h-36 bg-blue-600 rounded-[3.5rem] flex items-center justify-center shadow-2xl shadow-blue-100 group-active:scale-95 transition-all">
            ...
            <div className="w-full space-y-5 max-w-xs pb-10">
              ...
              <button onClick={() => setView('upload')} className="w-full bg-white border-2 border-slate-100 text-slate-600 p-6 rounded-[2.5rem] active:scale-95 transition-all flex items-center justify-center gap-3 font-black text-xl italic">上传图片</button>
            </div>
          </div>
        )}
        ...
        {view === 'processing' && (
          <div className="flex flex-col items-center justify-center h-full space-y-10 bg-white absolute inset-0 z-20 animate-in fade-in">
             ...
             <div className="text-center space-y-3">
                ...
                <p className="text-slate-400 font-bold text-sm tracking-widest uppercase opacity-50">AI Vision Engine Processing</p>
             </div>
          </div>
        )}
              ...
              {matchResult ? (
                <>
                  <div className="text-center text-white drop-shadow-2xl py-6 animate-in zoom-in">
                    ...
                    <p className="text-[10px] opacity-50 font-black tracking-[0.3em] uppercase">Matches Success · {processingTime.toFixed(2)}s</p>
                  </div>
                  {matchResult.feedback.map((fb: any, i: number) => (
                    <div key={i} className="w-full bg-white/95 backdrop-blur-xl rounded-[3rem] p-8 shadow-2xl animate-in slide-in-from-bottom duration-700">
                      ...
                      {fb.type === 'image' && <img src={fb.content} className="w-full rounded-[2rem] shadow-sm" referrerPolicy="no-referrer" alt="img" />}
                      {/* 🛑 核心细节：带声音自动播放 */}
                ...
                <div className="bg-white/95 backdrop-blur-xl p-12 rounded-[4rem] text-center space-y-8 shadow-2xl animate-in zoom-in">
                   ...
                   <div className="space-y-2"><p className="text-3xl font-black text-slate-800 italic tracking-tighter">未匹配到目标</p><p className="text-slate-400 font-medium text-xs break-all opacity-80 px-4">{errorMsg}</p></div>
...
const CameraView = ({ onCapture, onClose }: any) => {
  ...
  return (
    <div className="fixed inset-0 bg-black z-50 flex flex-col h-dvh overflow-hidden animate-in fade-in">
...
const UploadView = ({ onUpload, onClose }: any) => {
  ...
  return <div className="fixed inset-0 bg-white z-50 flex flex-col items-center justify-center p-10 animate-in fade-in"><Loader2 className="w-12 h-12 text-blue-600 animate-spin mb-4" /><p className="font-black italic">正在调起相册...</p></div>;
};
