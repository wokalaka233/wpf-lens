...
import { Camera, Upload, Settings, X, RefreshCw, Trash2, Plus, AlertCircle, ChevronLeft, Image as ImageIcon, Video, Music, Sparkles, PlayCircle, Loader2 } from 'lucide-react';
...
export default function App() {
  ...
  const handleAnalyze = async (base64: string) => {
    setCapturedImage(base64); // 存下照片用于缩略图和背景
    setView('processing');
        ...
        {view === 'processing' && (
          <div className="flex flex-col items-center justify-center h-full space-y-10 bg-white absolute inset-0 z-20 animate-in fade-in">
             <div className="relative w-40 h-40 flex items-center justify-center">
                ...
                {capturedImage && (
                  <img src={capturedImage} className="w-32 h-32 object-cover rounded-full shadow-lg border-4 border-white" alt="preview" />
                )}
             </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6 animate-in fade-in duration-500">
      {capturedImage && (
        <div className="absolute inset-0 z-0 overflow-hidden">
           <img src={capturedImage} className="w-full h-full object-cover scale-125 blur-[60px] opacity-70" alt="bg" />
        ...
        {result ? (
          <>
            <div className="text-center text-white drop-shadow-2xl py-6">
              <h2 className="text-5xl font-black mb-2 italic tracking-tighter">{result.name}</h2>
            ...
            {result.feedback.map((fb: any, i: number) => (
              <div key={i} className="w-full bg-white/90 backdrop-blur-2xl rounded-[3rem] p-8 shadow-2xl animate-in slide-in-from-bottom duration-700">
                ...
                {fb.type === 'image' && <img src={fb.content} className="w-full rounded-[2rem] shadow-md" referrerPolicy="no-referrer" alt="feedback" />}
            ...
            <button onClick={onClose} className="w-full bg-white/20 hover:bg-white/30 text-white p-6 rounded-[2.5rem] font-black text-2xl backdrop-blur-3xl border border-white/20 transition-all active:scale-95 shadow-2xl mt-4">完成确认</button>
          </>
        ) : (
          <div className="bg-white/95 backdrop-blur-3xl p-12 rounded-[4rem] text-center space-y-8 shadow-2xl animate-in zoom-in">
             ...
             <div className="space-y-2"><p className="text-3xl font-black text-slate-800 italic tracking-tighter">未匹配到目标</p><p className="text-slate-400 font-medium text-xs break-all opacity-80">{error}</p></div>
             <button onClick={onClose} className="w-full bg-slate-900 text-white py-6 rounded-[2.5rem] font-black text-xl">返回重试</button>
          </div>
        )}
      </div>
    </div>
...
const AdminPanel = ({ rules, onBack }: any) => ( <div className="h-full bg-white p-10 flex flex-col items-center justify-center"><button onClick={onBack} className="p-4 bg-slate-100 rounded-2xl font-bold">返回</button><p className="mt-20 text-center font-black italic opacity-20 italic">Rules Manager Active</p></div> );
const AdminLoginView = ({ onSuccess, onBack }: any) => ( <div className="h-dvh flex flex-col items-center justify-center p-10 bg-white"><input type="password" pattern="[0-9]*" inputMode="numeric" placeholder="ADMIN CODE" className="w-full p-8 bg-slate-50 rounded-[2.5rem] text-center font-black tracking-[0.5em] text-2xl mb-6 outline-none focus:ring-4 ring-blue-50" onChange={e => e.target.value === '11335510' && onSuccess()} /><button onClick={onBack} className="text-slate-300 font-bold">CANCEL</button></div> );
const UploadView = ({ onUpload, onClose }: any) => { useEffect(() => { const input = document.createElement('input'); input.type = 'file'; input.accept = 'image/*'; input.onchange = (e: any) => { const f = (e.target as HTMLInputElement).files?.[0]; if(f){ const r = new FileReader(); r.onload=ev=>onUpload(ev.target?.result as string); r.readAsDataURL(f); } else onClose(); }; input.click(); }, [onUpload, onClose]); return <div className="fixed inset-0 bg-white z-50 flex flex-col items-center justify-center p-10"><Loader2 className="w-12 h-12 text-blue-600 animate-spin mb-4" /><p className="font-black italic">正在调起相册...</p></div>; };
