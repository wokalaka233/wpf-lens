  ...
  return (
    <div className="h-dvh bg-white flex flex-col items-center justify-center p-8 space-y-8 animate-in fade-in">
      ...
      <div className="text-center">
        <h2 className="text-3xl font-bold text-slate-800">管理员认证</h2>
        <p className="text-slate-400 text-xs font-bold mt-2 tracking-widest uppercase">请输入管理密码</p>
      </div>
      <input 
        type="password" pattern="[0-9]*" inputMode="numeric" autoFocus
        className="w-full max-w-xs p-8 bg-slate-50 rounded-[2.5rem] text-center text-4xl font-bold tracking-[0.5em] focus:ring-4 ring-blue-50 outline-none transition-all"
        onChange={e => e.target.value === '11335510' && onSuccess()} 
      />
      <button onClick={onBack} className="text-slate-300 font-bold">返回首页</button>
    </div>
  );
};

// --- 2. 管理后台主面板 (全中文/圆润/无白屏) ---
  ...
  if (viewMode === 'form') return (
    <div className="h-full bg-white flex flex-col p-6 space-y-6 overflow-y-auto pb-24 animate-in slide-in-from-bottom">
      ...
      <div className="space-y-6">
        ...
        <button onClick={async () => { await storageService.saveRule(form); onBack(); }} className="w-full bg-blue-600 text-white p-6 rounded-[2.5rem] font-bold shadow-xl active:scale-95 transition-all text-lg">保存并发布规则</button>
      </div>
    </div>
  );
    ...
    <div className="h-full flex flex-col p-6 bg-white overflow-hidden animate-in fade-in">
      ...
      <div className="flex-1 overflow-y-auto space-y-4 scrollbar-hide pb-20">
        <button onClick={() => { setForm({ name: '', targetValue: '', feedback: [{type:'text', content:''}] }); setViewMode('form'); }} className="w-full p-10 border-4 border-dashed border-slate-100 rounded-[3.5rem] flex flex-col items-center justify-center gap-2 text-slate-300 hover:text-blue-400 transition-all">
          <Plus className="w-8 h-8" /><span className="font-bold text-xs tracking-widest">新增识别规则</span>
        </button>
        {(rules || []).map(rule => (
...
// --- 3. 反馈结果界面 (全屏覆盖/弱虚化/无斜体) ---
const FeedbackView = ({ result, error, capturedImage, processingTime, onClose }: any) => {
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 animate-in fade-in duration-300">
      {/* 🛑 核心需求：背景照片弱虚化 (blur-lg) */}
      {capturedImage && (
        <div className="absolute inset-0 z-0 overflow-hidden">
           <img src={capturedImage} className="w-full h-full object-cover scale-110 blur-lg opacity-70" alt="背景" />
          ...
          <>
            ...
            {result.feedback.map((fb: any, i: number) => (
              <div key={i} className="w-full bg-white/95 backdrop-blur-xl rounded-[3rem] p-8 shadow-2xl animate-in slide-in-from-bottom duration-700">
                ...
                {fb.type === 'image' && <img src={fb.content} className="w-full rounded-[2rem] shadow-sm" referrerPolicy="no-referrer" alt="反馈图" />}
...
export default function App() {
  ...
  const loadData = async () => {
    try {
      ...
      setRules([...GLOBAL_RULES, ...(Array.isArray(cloud) ? cloud.filter(c => !GLOBAL_RULES.find(g => g.id === c.id)) : [])]);
    } catch(e) { setRules(GLOBAL_RULES); }
  };
  ...
  return (
    <div className="h-dvh bg-white text-slate-900 font-sans max-w-lg mx-auto relative overflow-hidden flex flex-col">
      {/* 首页页头：仅在非反馈模式下显示，或被 FeedbackView 的 z-index 覆盖 */}
      ...
      <main className="flex-1 overflow-y-auto relative">
        {view === 'home' && (
          <div className="flex flex-col items-center justify-center h-full px-10 space-y-12 animate-in fade-in duration-700 pb-safe">
            ...
            <div className="text-center"><h2 className="text-4xl font-bold text-slate-800 tracking-tighter">准备扫描</h2></div>
        ...
        {view === 'processing' && (
          <div className="flex flex-col items-center justify-center h-full space-y-10 bg-white absolute inset-0 z-[120] animate-in fade-in">
             ...
             <div className="text-center space-y-3">
                ...
                <p className="text-slate-400 font-bold text-sm tracking-widest uppercase opacity-50">AI 识别中</p>
             </div>
          </div>
        )}
...
const UploadView = ({ onUpload, onClose }: any) => {
  ...
  return <div className="fixed inset-0 bg-white z-[130] flex flex-col items-center justify-center p-10"><Loader2 className="w-12 h-12 text-blue-600 animate-spin mb-4" /><p className="font-bold">正在选择照片...</p></div>;
};
